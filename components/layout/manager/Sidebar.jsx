...
const SideBar = ({ children }) => {
  ...
  const { employee } = useManagerEmployeeStore();
    ...
    <div className="flex min-h-screen">
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <header className="bg-white h-16 px-4 md:px-6 flex items-center justify-between border-b shadow-sm relative">
          ...
          <div className="flex items-center gap-4 ml-auto">
            ...
            <div className="flex items-center gap-2  pl-4 ml-2">
              ...
              <span className="text-sm font-medium hidden sm:block capitalize">
                {employee?.basicDetails?.name || "Admin_username"}
              </span>
            </div>
          </div>
          ...
          {/* User Dropdown */}
          {userMenuOpen && (
            <div className="absolute top-13 right-6 z-50 w-64 bg-white rounded-xl shadow-2xl border border-gray-100 overflow-hidden">
              <div className="p-4 border-b bg-gray-50">
                <p className="font-semibold text-gray-800 capitalize">
                  {employee?.basicDetails?.name || "Admin_username"}
                </p>
                <p className="text-xs text-gray-500 mt-0.5 truncate">
                  {employee?.basicDetails?.email || "admin2@promozionebranding.com"}
                </p>
              </div>
