...
import {
  createAdminReminderService,
  getAdminReminderService,
...
const AdminReminder = () => {
  ...
  const fetchReminders = async () => {
    try {
      setLoading(true);
      const res = await getAdminReminderService(adminDetail?._id);
      console.log(res?.data?.reminder, "res");
  ...
  const handleCreate = async () => {
    try {
      ...
      const res = await createAdminReminderService({
  ...
  return (
    <div className="bg-[#f3eaea] rounded-lg shadow-md p-6 h-[70vh] lg:h-[68vh] overflow-hidden">
      {/* Header */}
      ...
      <div className="flex flex-col md:flex-row gap-3 mb-6 w-full">
        <Input
          ...
          className={"border w-[60vw] border-black"}
        />
        ...
        <Input
          ...
          className={"border w-[40vw] border-black"}
        />
